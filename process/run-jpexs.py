...
def furnace_actionscript(abcfiles, dout):
    ...
    for bytecode in abcfiles:
        ...
        decompilecmd = 'furnace-avm2-decompiler -d -i %s > %s' % (os.path.join(dout, bytecode), os.path.join(dout, name))
...
def jpexs_extract(fin, dirname):
    ...
    javacmd = 'java -Djava.awt.headless=true -jar /Users/honey/src/work/pdf/thisneedsacoolname/ffdec.jar -export script %s %s' % (dout, fname)
